...
#Comprovem host
if [ "$IP" != "172.31.0.250/24" ]; then
	echo -e "[*]  ${RED}Ip Host Incorrecte!! -- ${IP} enlloc de 172.31.0.250/24${NONE}"
	echo -e "[*]  Ip host Incorrecte!! ${IP} ---  172.31.0.250/24" >> /var/log/examen.txt
else
	punts=$((punts + 4));
	echo -e "[*]  ${GREEN}La ip host és correcte${IP} ---  172.31.0.250/24 ${NONE}"
	echo -e "[*]  La ip host és correcte${IP} ---  172.31.0.250/24" >> /var/log/examen.txt
fi
...
if [ $(dpkg-query -W -f='${Status}' apache2 2>/dev/null | grep -c "ok installed") -eq 0 ] >>/var/log/examen.txt;
	...
	echo -e "[*]  Apache2 no està instal.lat:                            Incorrecte!!${NONE}" >> /var/log/examen.txt
	else
#	punts=$((punts + 1));
...
if [ $(dpkg-query -W -f='${Status}' mariadb-server 2>/dev/null | grep -c "ok installed") -eq 0 ];
	...
	echo -e "[*]  Mariadb-Server no està instal.lat                      Incorrecte!!" >> /var/log/examen.txt
	else
#	punts=$((punts + 1));
...
if [ $(dpkg-query -W -f='${Status}' php 2>/dev/null | grep -c "ok installed") -eq 0 ];
	...
	echo -e "[*]  PHP no està instal.lat:                                Incorrecte!!" >> /var/log/examen.txt
	else
#	punts=$((punts + 3));
...
if [ $(dpkg-query -W -f='${Status}' php-mysql 2>/dev/null | grep -c "ok installed") -eq 0 ];
	...
	echo -e "[*]  El paquet PHP-MYSQL no està instal.lat:                Incorrecte!!" >> /var/log/examen.txt
	else
	punts=$((punts + 1));
...
if [ -d "/var/lib/mysql/$DBNAME" ]; then
	...
	echo -e "[*]  La base de dades $DBNAME no existeix.                ${RED}Incorrecte!! ${NONE}"
	echo -e "[*]  La base de dades $DBNAME no existeix:                Incorrecte!!" >> /var/log/examen.txt
fi
...
if [ ! -d "$HOST_DIR" ];
	then echo -e "[*]  El directori host no existeix.                    ${RED}Incorrecte!!${NONE}"
	echo -e "[*]  El directori host no existeix:                         Incorrecte!!" >> /var/log/examen.txt
	else
	if [  "$(ls $HOST_DIR)" ];
...
#		punts=$((punts + 1));
		...
		echo -e "[*]  Fitxer index.html borrat:                                   Correcte!!" >> /var/log/examen.txt
		else 
		punts=$((punts -1));
		echo -e "[*]  No has esborrat el fitxer index.html!!                 ${RED}Incorrecte!! $punts${NONE}"
		echo -e "[*]  No has esborrat el fitxer index.html!!                 Incorrecte!! $punts" >> /var/log/examen.txt
		fi
	else
	echo -e "[*]  El directori host està buit.                               ${RED}Incorrecte!!${NONE}" 
	echo -e "[*]  El directori host està buit:                               Incorrecte!!" >> /var/log/examen.txt
fi
fi
...
HOST_DIR="/var/www/html/"
if [  "$(ls $HOST_DIR)" ];
	then echo -e "Comprovació dels permisos"
	if [ -d "$HOST_DIR" ] && [ $(stat -c "%a" $HOST_DIR) == "755" ]; then
#		punts=$((punts + 1));
		echo -e "[*]  El directori host té permisos correctes.               ${GREEN}Correcte!! $punts${NONE}"
		echo -e "[*]  El directori host té permisos correctes:               Correcte!!" >> /var/log/examen.txt
		else
		echo -e "[*]  El directori host no té permisos 755.                  ${RED}Incorrecte!!${NONE}"
		echo -e "[*]  El directori host no té permisos 755:                  Incorrecte!!" >> /var/log/examen.txt
	fi
fi
...
if [  "$(ls $HOST_DIR)" ];
	then echo -e "Comprovació d'usuari fitxer"
	if [ -d "$HOST_DIR" ] && [ $(stat -c "%U" $HOST_DIR) == "www-data" ]; then
#		punts=$((punts + 1));
		echo -e "[*]  El directori host té usuari correcte.                  ${GREEN}Correcte!! ${NONE}"
		echo -e "[*]  El directori host té usuari correcte:                  Correcte!!"  >> /var/log/examen.txt
	else
		echo -e "[*]  El directori host té usuari incorrecte.                ${RED}Incorrecte!!${NONE}"
		echo -e "[*]  El directori host té usuari incorrecte:                Incorrecte!!" >> /var/log/examen.txt
	fi
fi

if [  "$(ls $HOST_DIR)" ];
	then echo -e "Comprovació grup fitxer"
	if [ -d "$HOST_DIR" ] && [ $(stat -c "%G" $HOST_DIR) == "www-data" ]; then
		...
		echo -e "[*]  El directori host no té grup correcte                  ${RED}Incorrecte!!${NONE}"
		echo -e "[*]  El directori host no té grup correcte:                 Incorrecte!!" >> /var/log/examen.txt
	fi
fi
echo "+---------------------------------------------------------------------+"
echo "+---------------------------------------------------------------------+"
